---
title: "Analaysis of Oral Mucosal 16S dataset"
...
#load packages
cran_packages <- c("reshape2", "ggplot2", "vegan", "stringr", "gridExtra", "ape", "RColorBrewer", "dplyr", "knitr","cowplot","openxlsx", "circlize")
...
path.in <- "~/DecontamManuscript/Analyses/OralContamination" # CHANGE ME
...
## Assign taxonomy by classifying ASVs using the SILVA v128 database
This could be refactored into the preprocessing Rmd, which used an earlier version of the Silva database that will be replaced.
```{r}
...
## Plot patterns of taxa showing the inverse and flat frequency patterns in oral taxa (Figure 2a)
```{r}
...
## Call contaminants using isContaminant
```{r}
...
## Load reference database files & organize genera from the reference databases
```{r}
...
histo.wt
histo.wt.log <- histo.wt + scale_y_log10() + 
...
```{r}
fig2 <- plot_grid(f2a, histo, nrow=2, rel_heights = c(0.8,1), labels = c('a','b')) #export as 5x8" PDF
fig2
ggsave(file.path(path.out, "Oral_Fig2.pdf"), fig2, width=7, height=4.2, units="in", useDingbats=FALSE)
ggsave(file.path(path.out, "Oral_Fig2.png"), fig2, width=7, height=4.2, units="in")
```
Make combined publication version of Figure 3 (accuracy on the oral dataset):
```{r}
fig3 <- plot_grid(scat, proc, nrow=2, rel_heights = c(1,1), labels = c('a','b')) #export as 5x8" PDF
fig3
...
## Response to Reviewers: Frequency method FP rate
...
## Response to Reviewers: Constancy of contaminant concentrations across samples
...
sd$uL_to_add[sd$uL_to_add < 0 | sd$uL_to_add > 110] <- 110

#total reads/uL vs. quant_reading
p <- ggplot(sd, aes(x=quant_reading, y=sample_sums_total/uL_to_add, color=Subject))
p2 <- p + 
  geom_point(alpha=0.7) + 
  labs(title='Total reads', y='Total Reads / uL') + guides(shape=FALSE, color=FALSE)
p2

#contam reads/uL vs. quant_reading
p <- ggplot(sd, aes(x=quant_reading, y=log10(sample_sums_contam/uL_to_add), color=Subject))
p3 <- p + 
  geom_point(alpha=0.7) + 
  labs(title='Contaminant reads only', y='log10(Contaminant Reads / uL)')
p3

pconst <- plot_grid(p2,p3, nrow=1, rel_widths = c(1,1.3)) #save as 4x8.5 inch pdf
pconst
