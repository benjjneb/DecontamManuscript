...
f2a <- plot_abundance(MUC,c('Seq3','Seq152','Seq53','Seq1','Seq12','Seq200'),"quant_reading",taxa_are_rows=FALSE) + facet_wrap(~taxa_to_plot,nrow=1) + labs(x= 'log(DNA concentration)', y = 'log(Frequency)', color='', size=1) + geom_point(aes(color=Sample_or_Control)) +
   theme(axis.text.x = element_text(colour="grey20",size=FIG2_TEXTSIZE,angle=0,
                                    hjust=.5,vjust=.5,face="plain"),
        axis.text.y = element_text(colour="grey20",size=FIG2_TEXTSIZE,angle=0,
                                   hjust=1,vjust=0,face="plain"),  
        axis.title.x = element_text(colour="grey20",size=FIG2_TEXTSIZE,angle=0,
                                    hjust=.5,vjust=0,face="plain"),
        axis.title.y = element_text(colour="grey20",size=FIG2_TEXTSIZE,angle=90,
                                    hjust=.5,vjust=.5,face="plain"),
        strip.text.x = element_text(size = FIG2_TEXTSIZE, angle = 0),
        legend.title = element_text(size = FIG2_TEXTSIZE, angle = 0),
        legend.text = element_text(size = FIG2_TEXTSIZE, angle = 0),
        legend.position = 'bottom')

f2a
```

#Figure 2b, Figure S2: examine Ps of taxa in oral dataset
```{r}
...
f2b <- histo + geom_histogram() + labs(x = 'decontam P', y='Number ASVs') + facet_wrap(~Group) + scale_fill_manual(values=cbbPalette) +
   theme(axis.text.x = element_text(colour="grey20",size=FIG2_TEXTSIZE,angle=0,
                                    hjust=.5,vjust=.5,face="plain"),
        axis.text.y = element_text(colour="grey20",size=FIG2_TEXTSIZE,angle=0,
                                   hjust=1,vjust=0,face="plain"),  
...
#combine 2a and 2b
f2 <- plot_grid(f2a, f2b, labels='auto',nrow=2)
f2
# ggsave("~/Desktop/Contamination/Figures/Fig2.pdf", f2, width=7, height=5, units="in", useDingbats=FALSE)

#S2, without log 
s2 <- ggplot(TAXann, aes(pval.c, fill=Group))
fs2a <- s2 + 
  geom_histogram(aes(weight=TAXsums)) + 
  labs(x = 'decontam P', y='Number Reads') + 
...
fs2 <- plot_grid(fs2a,fs2b, labels='auto', nrow=2,rel_heights=c(1,1.25))
fs2
# ggsave("~/Desktop/Contamination/Figures/FigS2.pdf", fs2, width=7, height=5, units="in", useDingbats=FALSE)
```

#Table S1: Blacklist taxa with high Ps
```{r}
#what are the blacklisters with high Ps?
...
plot_abundance(MUC, oralmaybes, "quant_reading", taxa_are_rows=FALSE) +
  ...
  labs(title='Blacklist taxa with high P')
```
